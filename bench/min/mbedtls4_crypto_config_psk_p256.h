/* mbedTLS 4.x PSA config: minimal TLS 1.3 PSK + ECDHE client crypto.
 * Kept: ECDH (secp256r1 (P-256) only), AES-128-GCM, SHA-256, HKDF/HMAC, external RNG.
 * Dropped vs the cert config: all RSA, all ECDSA, RSA/ECC public-key verify.
 * Variant (b): secp256r1 (P-256) key exchange only. */

#ifndef WN_FP_MB4_CRYPTO_CONFIG_PSK_P256_H
#define WN_FP_MB4_CRYPTO_CONFIG_PSK_P256_H
...
/* no PEM/base64, no key/CSR writing */
...
/* drop all public-key signature / encryption algs (no cert stack) */
...
/* drop unused KDFs / PRFs (keep HKDF for TLS 1.3) */
...
/* drop unused hashes (keep SHA-256 only) */
...
/* ---- curves: keep secp256r1 (P-256) only ---- */
...
/* PSA_WANT_ECC_SECP_R1_256 stays enabled from the stock config */
...
/* no cert / signatures: drop all RSA key types */
...
#endif /* WN_FP_MB4_CRYPTO_CONFIG_PSK_P256_H */
