...
/**
 * wolfNano crypto-floor self-test, run on an emulated Cortex-M33 (STM32H563)
...
{
    emu_uart_init();
    printf("wolfNano m33mu floor self-test (Cortex-M33 / STM32H563)\n");

    kat_sha256();
    kat_aesgcm();

    if (g_fail == 0) {
        printf("wolfNano floor: all KATs pass\n");
        __asm volatile("bkpt #0x7e");
    }
    else {
        printf("wolfNano floor: KAT FAILURE\n");
