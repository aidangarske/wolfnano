#!/bin/sh
# Build the wolfNanoTLS STM32H563 image and run it under m33mu, gating on the
...
grep -q "wolfNanoTLS floor: all KATs pass" "$LOG"
grep -q "\[EXPECT BKPT\] Success" "$LOG"
echo "m33mu: PASS (wolfNanoTLS floor on emulated Cortex-M33)"
