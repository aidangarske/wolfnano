...
wolfNano's own size and speed numbers live in [Footprint](Footprint.md) and
...
`ECP_WINDOW_SIZE 2`) so the comparison is not inflated in wolfNano's favor. Both
...
binder) and present on both. The remaining gap is architectural: wolfNano uses
...
- **Exact configs:** `bench/min/mbedtls_config_psk_hardmin.h` +
  ...
  `configs/user_settings_*.h` (wolfNano).
...
wolfSSL also ship **no ML-KEM / ML-DSA**, so wolfNano's PQC client rows have no
counterpart.
...
The complete wolfNano TLS 1.3 client is roughly 6x smaller in compiled `.text`
